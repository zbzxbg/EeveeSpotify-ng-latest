...
/// 语义化版本（SemVer）比较。
///
/// 为什么不用"按 `.` 切三段比大小"那种土办法（本仓库原来就是那套）：
///   · 远程 tag 有 `6.4.3` / `v6.4.3` / `swift6.4.3` / `6.4.3-rc.1` 各种写法；
///   · 预发布版（`-rc.1`）按 SemVer 规则**小于**同号正式版，而"三段比大小"会把它
///     判成相等甚至更大 —— 于是要么漏提示、要么误提示。
///
/// ⚠️ 这套实现与 `EeveeSpotifyReborn-ng` **逐字一致**（含 `+build` 元数据剥离、
/// 预发布标识符的数字/文本混合比较规则）。要改一起改，别只改一边。
...
struct EeveeSettingsVersionView: View {
    ...
    //
    // ⚠️ 这套「不抛错 + 失败即无更新」的做法来自 `EeveeSpotifyReborn-ng`：
    // 以前这里用 `try await` + `latestVersion` 留 nil 的方式，网络一出问题
    // 就永远停在转圈上（`if latestVersion == nil` 那个分支）。
